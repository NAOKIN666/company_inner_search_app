...
import pandas as pd # CSVファイルの表示用に追加
...
def frame_display():
    ...
    with st.sidebar:
        display_select_mode()
        st.divider()
        display_initial_ai_message_side()
...
def display_initial_ai_message_main():
    """
    AIメッセージの初期表示
    """
...
def display_initial_ai_message_side():
    """
    利用目的の詳細説明
    """
    with st.container():
...
def display_search_llm_response(llm_response):
    # st.write("✅ 関数が呼び出されました")
    # st.write("🟢 context:", llm_response["context"])
    # st.write("🔵 context 件数:", len(llm_response["context"]))
    """
...
# -CSV処理用に追加（ここから）-------------------------------------------
        if main_file_path.endswith(".csv"):
            try:
                # Pandasでcsvファイルを読み込む
                df = pd.read_csv(main_file_path)

                # 部署の一覧を取得
                department_list = df['部署'].unique().tolist()

                # 部署名を抽出
                question = llm_response.get("question", "")
                llm = ChatOpenAI(temperature=0, model=ct.MODEL_NAME)
                parser = StrOutputParser()
                template = f"""以下の部署リストの中から、ユーザーの質問に最も関連する部署名を一つだけ選んでください。
                関連する部署名がなければ「全件」と答えてください。

                部署リスト: {department_list}

                質問: {{question}}"""

                prompt_template = ChatPromptTemplate.from_template(template)
                chain = prompt_template | llm | parser
                department_name = chain.invoke({"question": question})

                # 部署名でデータを絞り込み
                if department_name != "全件" and "部署" in df.columns:
                    filtered_df = df[df["部署"] == department_name].copy()
                    title = f"「{department_name}」に所属している従業員情報"
                else:
                    filtered_df = df.copy()
                    title = "従業員情報一覧"
                ...
                if not filtered_df.empty:
                    st.markdown(f"##### {title}")
                    # filtered_df をそのまま渡すと、全項目がCSVの順で表示される
                    st.dataframe(filtered_df, use_container_width=True, hide_index=True)
                else:
                    st.warning(f"「{department_name}」に該当する従業員情報は見つかりませんでした。")

                # 後続の処理は不要なため、会話ログ用のデータを作成して処理を終了
                content = {
                    "mode": ct.ANSWER_MODE_2, # 社内問い合わせモード
                ...
                st.error(f"CSVファイルの読み込みに失敗しました: {e}", icon=ct.ERROR_ICON)
...
# -CSV処理用に追加（ここまで）-------------------------------------------
        ...
        # st.write("🟡 main_file_path:", main_file_path)
        ...
        for document in llm_response["context"][1:]:
            ...
            # st.write("🟠 now checking sub_file_path:", sub_file_path)
            ...
            # st.write(f"sub_choices: {sub_choices}")
